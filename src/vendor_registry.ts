  ...
  for (const [slug, cfg] of Object.entries(registry.vendors ?? {})) {
    ...
    for (const rule of cfg.patterns ?? []) {
      if (UNSAFE_RE.test(rule.match)) {
        getLogger().warn(`[Apidepth] Skipping unsafe pattern for ${sanitizeLog(slug)}: ${rule.match}`);
        continue;
      }
      try {
        list.push([new RegExp(rule.match), rule.replace]);
      } catch (e) {
        getLogger().warn(`[Apidepth] Skipping invalid pattern for ${sanitizeLog(slug)} ${rule.match}: ${e}`);
      }
    }
    m.set(slug, list);
  }
...
export const VendorRegistry = {
  ...
  get version(): string    { return _version; },
  get vendorCount(): number { return new Set(_hosts.values()).size; },
};
