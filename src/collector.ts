...
const PRIVATE_HOST_RE =
  /^localhost$|^127\.|^0\.0\.0\.0$|^0$|^169\.254\.|^10\.|^172\.(1[6-9]|2\d|3[01])\.|^192\.168\.|^\[?::1\]?$|^\[?f[cd]|^\[?fe80:/i;
      ...
      withSkip(() => {
        const req = https.request(
          ...
          (res) => {
            ...
            const ok =
              res.statusCode !== undefined && res.statusCode >= 200 && res.statusCode <= 299;
            if (ok) {
              resolve();
            } else {
              reject(
                new Error(
                  `Collector returned HTTP ${res.statusCode} — verify your api_key and collector_url`
                )
              );
            }
          }
...
const _sdkMetadata: Record<string, unknown> = {
