...
export function loadAndStart(): void {
  ...
  _refreshTimer = setInterval(async () => {
    ...
    if (registry) {
      VendorRegistry.replace(registry, getConfiguration().extraVendors);
...
async function _bootstrap(): Promise<void> {
  ...
  if (source) {
    if (!remote) {
      // Disk path: _fetchRemote never ran, so conflict detection and warnings fire here.
      _detectConflicts(source);
      _emitWarnings(source);
    }
    VendorRegistry.replace(source, getConfiguration().extraVendors);
    _applyCustomerVendors(source);
  }
}
        ...
        (res) => {
          ...
          res.on("data", (chunk: Buffer) => {
            ...
            if (totalBytes > MAX_RESPONSE_BYTES) {
              getLogger().warn(
                `[Apidepth] Registry response too large (${totalBytes} bytes) — skipping`
              );
              res.destroy();
              resolve(null);
            } else {
...
// Populates _conflictVendors — called inside _fetchRemote before _emitWarnings.
function _detectConflicts(registry: RegistryJson): void {
...
// Loads customer_vendors hosts into the registry — called after VendorRegistry.replace
// so the hosts survive the replace's full rebuild of _hosts.
